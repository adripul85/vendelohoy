...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        // Authorization: Verify that the caller is the seller of this transaction
        if (decodedToken.uid !== txData.sellerId) {
            return res.status(403).json({ error: 'No tienes permisos para despachar esta orden' });
        }
